...
public extension Test_Entry {
  static func decode(json: Any) throws -> Test_Entry {
    ...
    var datetime_type: String? = Optional.none
    if let value = json["datetime_type"] {
      datetime_type = Optional.some(try decode_name(unbox(value, as: String.self), name: "datetime_type"))
    }
    ...
    var bytes_type: String? = Optional.none
    if let value = json["bytes_type"] {
      bytes_type = Optional.some(try decode_name(unbox(value, as: String.self), name: "bytes_type"))
    }
